...
/* Oracle reject logon procedure call */
int oracle_rejected_logon_procedureCall(const char *msg, const char *delim, 
                                       int64_t *rc, char *rm, size_t rm_size) {
    if (!msg || !delim || !rc || !rm) {
        set_last_error("Invalid parameters: msg, delim, rc, and rm cannot be NULL");
        return 0;
    }
    ...
    dpiStmt *stmt = NULL;
    dpiVar *vMsg = NULL, *vDelim = NULL, *vRc = NULL, *vRm = NULL;
    dpiData *dMsg = NULL, *dDelim = NULL, *dRc = NULL, *dRm = NULL;
    int result = 0;
    
    // Prepare SQL
    const char *sql = "BEGIN write_reject(:LOGMESSAGE, :DELIMITER, :RETURNCODE, :RETURNMESSAGE); END;";
    ...
    // IN VARCHAR2 (message)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, 4000, 10, 0, NULL, &vMsg, &dMsg) != DPI_SUCCESS) {
        set_last_error("Failed to create message variable");
        goto cleanup;
    }
    if (dpiVar_setFromBytes(vMsg, 0, msg, (uint32_t)strlen(msg)) != DPI_SUCCESS) {
        set_last_error("Failed to set message value");
        goto cleanup;
    }
    
    // IN VARCHAR2 (delimiter)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, 16, 10, 0, NULL, &vDelim, &dDelim) != DPI_SUCCESS) {
        set_last_error("Failed to create delimiter variable");
        goto cleanup;
    }
    if (dpiVar_setFromBytes(vDelim, 0, delim, (uint32_t)strlen(delim)) != DPI_SUCCESS) {
        set_last_error("Failed to set delimiter value");
        goto cleanup;
    }
    
    // OUT NUMBER (return code)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_NUMBER, DPI_NATIVE_TYPE_INT64,
                       100, 0, 10, 0, NULL, &vRc, &dRc) != DPI_SUCCESS) {
        set_last_error("Failed to create return code variable");
        goto cleanup;
    }
    
    // OUT VARCHAR2 (return message)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, 4000, 10, 0, NULL, &vRm, &dRm) != DPI_SUCCESS) {
        set_last_error("Failed to create return message variable");
        goto cleanup;
    }
    
    // Bind parameters by name
    if (dpiStmt_bindByName(stmt, "LOGMESSAGE", 10, vMsg) != DPI_SUCCESS) {
        set_last_error("Failed to bind message parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "DELIMITER", 9, vDelim) != DPI_SUCCESS) {
        set_last_error("Failed to bind delimiter parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "RETURNCODE", 10, vRc) != DPI_SUCCESS) {
        set_last_error("Failed to bind return code parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "RETURNMESSAGE", 13, vRm) != DPI_SUCCESS) {
        set_last_error("Failed to bind return message parameter");
        goto cleanup;
    }
    
    // Execute the procedure
    if (dpiStmt_execute(stmt, DPI_MODE_EXEC_DEFAULT, NULL) != DPI_SUCCESS) {
        set_last_error("Failed to execute write_reject procedure");
        goto cleanup;
    }
    
    // Get output parameters
    *rc = dRc[0].value.asInt64;
    
    // Copy return message safely
    if (dRm[0].value.asBytes.ptr && dRm[0].value.asBytes.length > 0) {
        size_t copy_len = (size_t)dRm[0].value.asBytes.length;
        if (copy_len >= rm_size) {
            copy_len = rm_size - 1; // Leave space for null terminator
        }
        memcpy(rm, dRm[0].value.asBytes.ptr, copy_len);
        rm[copy_len] = '\0';
    } else {
        rm[0] = '\0'; // Empty string if NULL or empty
    }
    
    result = 1; // Success
    set_last_error("write_reject procedure executed successfully");
    
cleanup:
    if (vMsg) dpiVar_release(vMsg);
    if (vDelim) dpiVar_release(vDelim);
    if (vRc) dpiVar_release(vRc);
    if (vRm) dpiVar_release(vRm);
...
/* Oracle session data procedure call */
int oracle_write_session_data_procedureCall(int64_t cmd_code,
                                           const char *session_id,
                                           const char *user_name,
                                           int64_t origin_state_id,
                                           const char *auth_request_type,
                                           const char *calling_station_id,
                                           const char *ip_address,
                                           const char *msisdn,
    ...
    if (!session_id || !user_name || !password_out) {
        set_last_error("Invalid parameters: session_id, user_name, and password_out cannot be NULL");
        return 0;
    }
    ...
    dpiStmt *stmt = NULL;
    dpiVar *vCmdCode = NULL, *vSessionId = NULL, *vUserName = NULL, *vOriginStateId = NULL;
    dpiVar *vAuthRequestType = NULL, *vCallingStationId = NULL, *vIpAddress = NULL, *vMsisdn = NULL;
    dpiVar *vServiceSelection = NULL, *vTerminationCause = NULL, *vInPass = NULL, *vPasswordOut = NULL;
    dpiData *dCmdCode = NULL, *dSessionId = NULL, *dUserName = NULL, *dOriginStateId = NULL;
    dpiData *dAuthRequestType = NULL, *dCallingStationId = NULL, *dIpAddress = NULL, *dMsisdn = NULL;
    dpiData *dServiceSelection = NULL, *dTerminationCause = NULL, *dInPass = NULL, *dPasswordOut = NULL;
    int result = 0;
    
    // Prepare SQL
    const char *sql = "BEGIN WRITE_SESSION_DATA(:P_CMD_CODE, :P_SESSION_ID, :P_USER_NAME, "
                      ":P_ORIGIN_STATE_ID, :P_AUTH_REQUEST_TYPE, :P_CALLING_STATION_ID, "
                      ":P_IP_ADDRESS, :P_MSISDN, :P_SERVICE_SELECTION, :P_TERMINATION_CAUSE, "
                      ":IN_PASS, :PASSWORD); END;";
    ...
    // Create bind variables for all parameters
    
    // P_CMD_CODE (NUMBER - IN)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_NUMBER, DPI_NATIVE_TYPE_INT64,
                       100, 0, 10, 0, NULL, &vCmdCode, &dCmdCode) != DPI_SUCCESS) {
        set_last_error("Failed to create cmd_code variable");
        goto cleanup;
    }
    dpiData_setInt64(dCmdCode, cmd_code);
    
    // P_SESSION_ID (VARCHAR2 - IN)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, 100, 10, 0, NULL, &vSessionId, &dSessionId) != DPI_SUCCESS) {
        set_last_error("Failed to create session_id variable");
        goto cleanup;
    }
    if (dpiVar_setFromBytes(vSessionId, 0, session_id, (uint32_t)strlen(session_id)) != DPI_SUCCESS) {
        set_last_error("Failed to set session_id value");
        goto cleanup;
    }
    
    // P_USER_NAME (VARCHAR2 - IN)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, 100, 10, 0, NULL, &vUserName, &dUserName) != DPI_SUCCESS) {
        set_last_error("Failed to create user_name variable");
        goto cleanup;
    }
    if (dpiVar_setFromBytes(vUserName, 0, user_name, (uint32_t)strlen(user_name)) != DPI_SUCCESS) {
        set_last_error("Failed to set user_name value");
        goto cleanup;
    }
    
    // P_ORIGIN_STATE_ID (NUMBER - IN)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_NUMBER, DPI_NATIVE_TYPE_INT64,
                       100, 0, 10, 0, NULL, &vOriginStateId, &dOriginStateId) != DPI_SUCCESS) {
        set_last_error("Failed to create origin_state_id variable");
        goto cleanup;
    }
    dpiData_setInt64(dOriginStateId, origin_state_id);
    
    // P_AUTH_REQUEST_TYPE (VARCHAR2 - IN)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, 50, 10, 0, NULL, &vAuthRequestType, &dAuthRequestType) != DPI_SUCCESS) {
        set_last_error("Failed to create auth_request_type variable");
        goto cleanup;
    }
    if (auth_request_type && strlen(auth_request_type) > 0) {
        if (dpiVar_setFromBytes(vAuthRequestType, 0, auth_request_type, (uint32_t)strlen(auth_request_type)) != DPI_SUCCESS) {
            set_last_error("Failed to set auth_request_type value");
            goto cleanup;
        }
    }
    
    // P_CALLING_STATION_ID (VARCHAR2 - IN)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, 100, 10, 0, NULL, &vCallingStationId, &dCallingStationId) != DPI_SUCCESS) {
        set_last_error("Failed to create calling_station_id variable");
        goto cleanup;
    }
    if (calling_station_id && strlen(calling_station_id) > 0) {
        if (dpiVar_setFromBytes(vCallingStationId, 0, calling_station_id, (uint32_t)strlen(calling_station_id)) != DPI_SUCCESS) {
            set_last_error("Failed to set calling_station_id value");
            goto cleanup;
        }
    }
    
    // P_IP_ADDRESS (VARCHAR2 - IN)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, 50, 10, 0, NULL, &vIpAddress, &dIpAddress) != DPI_SUCCESS) {
        set_last_error("Failed to create ip_address variable");
        goto cleanup;
    }
    if (ip_address && strlen(ip_address) > 0) {
        if (dpiVar_setFromBytes(vIpAddress, 0, ip_address, (uint32_t)strlen(ip_address)) != DPI_SUCCESS) {
            set_last_error("Failed to set ip_address value");
            goto cleanup;
        }
    }
    ...
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, 100, 10, 0, NULL, &vMsisdn, &dMsisdn) != DPI_SUCCESS) {
        set_last_error("Failed to create msisdn variable");
        goto cleanup;
    }
    if (msisdn && strlen(msisdn) > 0) {
        if (dpiVar_setFromBytes(vMsisdn, 0, msisdn, (uint32_t)strlen(msisdn)) != DPI_SUCCESS) {
            set_last_error("Failed to set msisdn value");
            goto cleanup;
        }
    }
    
    // P_SERVICE_SELECTION (VARCHAR2 - IN)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, 100, 10, 0, NULL, &vServiceSelection, &dServiceSelection) != DPI_SUCCESS) {
        set_last_error("Failed to create service_selection variable");
        goto cleanup;
    }
    if (service_selection && strlen(service_selection) > 0) {
        if (dpiVar_setFromBytes(vServiceSelection, 0, service_selection, (uint32_t)strlen(service_selection)) != DPI_SUCCESS) {
            set_last_error("Failed to set service_selection value");
            goto cleanup;
        }
    }
    
    // P_TERMINATION_CAUSE (VARCHAR2 - IN)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, 50, 10, 0, NULL, &vTerminationCause, &dTerminationCause) != DPI_SUCCESS) {
        set_last_error("Failed to create termination_cause variable");
        goto cleanup;
    }
    if (termination_cause && strlen(termination_cause) > 0) {
        if (dpiVar_setFromBytes(vTerminationCause, 0, termination_cause, (uint32_t)strlen(termination_cause)) != DPI_SUCCESS) {
            set_last_error("Failed to set termination_cause value");
            goto cleanup;
        }
    }
    
    // IN_PASS (VARCHAR2 - IN OUT, mivel nincs explicit IN kulcsszó a procedure-ben)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, 100, 1, 0, NULL, &vInPass, &dInPass) != DPI_SUCCESS) {
        set_last_error("Failed to create in_pass variable");
        goto cleanup;
    }
    // Beállítjuk az input értéket az első elemre (index 0)
    if (in_pass && strlen(in_pass) > 0) {
        dInPass[0].value.asBytes.ptr = (char*)in_pass;
        dInPass[0].value.asBytes.length = (uint32_t)strlen(in_pass);
        dInPass[0].isNull = 0;
    } else {
        dInPass[0].isNull = 1; // NULL érték ha nincs megadva
    }
    
    // PASSWORD (VARCHAR2 - OUT)
    if (dpiConn_newVar(conn, DPI_ORACLE_TYPE_VARCHAR, DPI_NATIVE_TYPE_BYTES,
                       100, (uint32_t)password_out_size, 1, 0, NULL, &vPasswordOut, &dPasswordOut) != DPI_SUCCESS) {
        set_last_error("Failed to create password_out variable");
        goto cleanup;
    }
    
    // Bind all parameters by name
    if (dpiStmt_bindByName(stmt, "P_CMD_CODE", 10, vCmdCode) != DPI_SUCCESS) {
        set_last_error("Failed to bind P_CMD_CODE parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "P_SESSION_ID", 12, vSessionId) != DPI_SUCCESS) {
        set_last_error("Failed to bind P_SESSION_ID parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "P_USER_NAME", 11, vUserName) != DPI_SUCCESS) {
        set_last_error("Failed to bind P_USER_NAME parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "P_ORIGIN_STATE_ID", 17, vOriginStateId) != DPI_SUCCESS) {
        set_last_error("Failed to bind P_ORIGIN_STATE_ID parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "P_AUTH_REQUEST_TYPE", 19, vAuthRequestType) != DPI_SUCCESS) {
        set_last_error("Failed to bind P_AUTH_REQUEST_TYPE parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "P_CALLING_STATION_ID", 20, vCallingStationId) != DPI_SUCCESS) {
        set_last_error("Failed to bind P_CALLING_STATION_ID parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "P_IP_ADDRESS", 12, vIpAddress) != DPI_SUCCESS) {
        set_last_error("Failed to bind P_IP_ADDRESS parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "P_MSISDN", 8, vMsisdn) != DPI_SUCCESS) {
        set_last_error("Failed to bind P_MSISDN parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "P_SERVICE_SELECTION", 19, vServiceSelection) != DPI_SUCCESS) {
        set_last_error("Failed to bind P_SERVICE_SELECTION parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "P_TERMINATION_CAUSE", 19, vTerminationCause) != DPI_SUCCESS) {
        set_last_error("Failed to bind P_TERMINATION_CAUSE parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "IN_PASS", 7, vInPass) != DPI_SUCCESS) {
        set_last_error("Failed to bind IN_PASS parameter");
        goto cleanup;
    }
    if (dpiStmt_bindByName(stmt, "PASSWORD", 8, vPasswordOut) != DPI_SUCCESS) {
        set_last_error("Failed to bind PASSWORD parameter");
        goto cleanup;
    }
    
    // Execute the procedure
    if (dpiStmt_execute(stmt, DPI_MODE_EXEC_DEFAULT, NULL) != DPI_SUCCESS) {
        // Részletes Oracle hibaüzenet lekérése
        pthread_mutex_lock(&g_pool_singleton.mutex);
        if (g_pool_singleton.pool.ctx) {
            dpiErrorInfo err;
            dpiContext_getError(g_pool_singleton.pool.ctx, &err);
            snprintf(g_pool_singleton.last_error, sizeof(g_pool_singleton.last_error),
                    "Failed to execute WRITE_SESSION_DATA procedure: %.*s", 
                    err.messageLength, err.message);
        } else {
            set_last_error("Failed to execute WRITE_SESSION_DATA procedure: context not available");
        }
        pthread_mutex_unlock(&g_pool_singleton.mutex);
        goto cleanup;
    }
    
    // Get output parameter (password)
    if (dPasswordOut[0].value.asBytes.ptr && dPasswordOut[0].value.asBytes.length > 0) {
        size_t copy_len = (size_t)dPasswordOut[0].value.asBytes.length;
        if (copy_len >= password_out_size) {
            copy_len = password_out_size - 1; // Leave space for null terminator
        }
        memcpy(password_out, dPasswordOut[0].value.asBytes.ptr, copy_len);
        password_out[copy_len] = '\0';
    } else {
        password_out[0] = '\0'; // Empty string if NULL or empty
    }
    
    result = 1; // Success
    set_last_error("WRITE_SESSION_DATA procedure executed successfully");
    
cleanup:
    if (vCmdCode) dpiVar_release(vCmdCode);
    if (vSessionId) dpiVar_release(vSessionId);
    if (vUserName) dpiVar_release(vUserName);
    if (vOriginStateId) dpiVar_release(vOriginStateId);
    if (vAuthRequestType) dpiVar_release(vAuthRequestType);
    if (vCallingStationId) dpiVar_release(vCallingStationId);
    if (vIpAddress) dpiVar_release(vIpAddress);
    if (vMsisdn) dpiVar_release(vMsisdn);
    if (vServiceSelection) dpiVar_release(vServiceSelection);
    if (vTerminationCause) dpiVar_release(vTerminationCause);
    if (vInPass) dpiVar_release(vInPass);
    if (vPasswordOut) dpiVar_release(vPasswordOut);
    if (stmt) dpiStmt_release(stmt);
...
const char* oracle_get_last_error(void) {
    pthread_mutex_lock(&g_pool_singleton.mutex);
    const char* error = g_pool_singleton.last_error;
    pthread_mutex_unlock(&g_pool_singleton.mutex);
    return error;
}
*/